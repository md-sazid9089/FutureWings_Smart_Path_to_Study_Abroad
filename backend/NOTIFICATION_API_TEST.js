...
async function testFetchNotifications(token) {
  ...
  try {
    ...
    const { notifications, total, limit, offset } = response.data.data;
    if (notifications && Array.isArray(notifications) && total !== undefined) {
    ...
    const allUnread = notifications.every((notif) => notif.read === false);
    if (allUnread) {
      log.pass(`All ${notifications.length} notifications are unread`);
      testsPassed++;
    } else {
    ...
    log.info(`Total unread: ${total} | Displaying: ${notifications.length}`);
